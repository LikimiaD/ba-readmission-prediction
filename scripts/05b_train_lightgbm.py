...
def main():
    ...
    plot_roc(y_val.to_numpy(), y_proba_val_honest, MODEL_NAME, FIGURES_DIR / 'roc' / 'lightgbm.png')
    plot_pr(y_val.to_numpy(), y_proba_val_honest, MODEL_NAME, FIGURES_DIR / 'pr' / 'lightgbm.png')
    plot_confusion(
        y_val.to_numpy(), y_proba_val_honest, MODEL_NAME,
        FIGURES_DIR / 'confusion' / 'lightgbm.png',
    )
    ...
    booster, categories, fit_seconds = lightgbm_model.fit_final(
        best_params, X_full, y_full, feature_types, num_boost_round=num_boost_round,
    )

    y_proba_refit = lightgbm_model.predict_proba(booster, X_val, feature_types, categories)
    ...
    joblib.dump(
        ...
        MODELS_DIR / 'lightgbm_final.pkl', compress=3,
    )
    ...
    print(
        f"\n[lightgbm] val ROC-AUC={m_val['roc_auc']:.4f} "
